/* global jQuery */
/* global wp */
function storepress_media_upload(button_class) {
    'use strict';
 ...
 *** Generate unique id ***
 *********************************************/
function storepress_customizer_repeater_uniqid(prefix, more_entropy) {
    'use strict';
 ...
 *** General Repeater ***
 *********************************************/
function storepress_customizer_repeater_refresh_social_icons(th) {
    'use strict';
    ...
    th.find('.customizer-repeater-social-repeater-container').each(function () {
        ...
        if (!id) {
            id = 'customizer-repeater-social-repeater-' + storepress_customizer_repeater_uniqid();
    ...
    storepress_customizer_repeater_refresh_general_control_values();
}


function storepress_customizer_repeater_refresh_general_control_values() {
    'use strict';
    jQuery('.customizer-repeater-general-control-repeater').each(function () {
        ...
        th.find('.customizer-repeater-general-control-repeater-container').each(function () {
            ...
            if (!id) {
                id = 'social-repeater-' + storepress_customizer_repeater_uniqid();
            ...
            if (text !== '' || image_url !== '' || title !== '' || subtitle !== '' || btn3 !== '' || icon_value !== '' || link !== '' || choice !== '' || social_repeater !== '' || shortcode !== '' || slide_align !== '' || color !== '') {
                values.push({
                    'icon_value': (choice === 'customizer_repeater_none' ? '' : icon_value),
                    'color': color,
                    'color2': color2,
                    'text': storepressescapeHtml(text),
                    'link': link,
                    'text2': storepressescapeHtml(text2),
					'button_second': storepressescapeHtml(button_second),
                    'link2': link2,
					'link3': link3,
                    'image_url': (choice === 'customizer_repeater_none' ? '' : image_url),
                    'choice': choice,
					'image_url2': image_url2,
                    'title': storepressescapeHtml(title),
                    'subtitle': storepressescapeHtml(subtitle),
					'subtitle2': storepressescapeHtml(subtitle2),
					'btn3': storepressescapeHtml(btn3),
					'slide_align': storepressescapeHtml(slide_align),
					'open_new_tab' : open_new_tab,
                    'social_repeater': storepressescapeHtml(social_repeater),
                    'id': id,
                    'shortcode': storepressescapeHtml(shortcode)
                });
            }
...
jQuery(document).ready(function () {
    'use strict';
    var storepress_theme_controls = jQuery('#customize-theme-controls');
    storepress_theme_controls.on('click', '.customizer-repeater-customize-control-title', function () {
    ...
    storepress_theme_controls.on('change', '.icp',function(){
        storepress_customizer_repeater_refresh_general_control_values();
        return false;
    });
	
	storepress_theme_controls.on('change','.customizer-repeater-slide-align', function(){
		storepress_customizer_repeater_refresh_general_control_values();
        return false;
	...
	storepress_theme_controls.on('change','.customizer-repeater-image2-control', function(){
		storepress_customizer_repeater_refresh_general_control_values();
        return false;
		
	});
	
    storepress_theme_controls.on('change', '.customizer-repeater-image-choice', function () {
    ...
    storepress_media_upload('.customizer-repeater-custom-media-button');
     ...
     *
     */
    storepress_theme_controls.on('click', '.customizer-repeater-new-field', function () {
        var split_add_more_button=jQuery(this).text();
        var split_add_more_button_split=split_add_more_button.substr(4, 12);
        var th = jQuery(this).parent();
        var id = 'customizer-repeater-' + storepress_customizer_repeater_uniqid();
		
		if(split_add_more_button_split=="Add New Info")
        {
        if(jQuery('#exist_storepress_Information').val()>=3)
        {
        jQuery(".customizer_storepress_header_info_upsale").show();
        return false;   
        }
         if(jQuery('#exist_storepress_Information').val()<3)
        {
         var new_service_add_val=parseInt(jQuery('#exist_storepress_Information').val())+1;
         jQuery('#exist_storepress_Information').val(new_service_add_val);  
        }
        }
		
		if(split_add_more_button_split=="Add New Slid")
        {
        if(jQuery('#exist_storepress_Slider').val()>=3)
        {
        jQuery(".customizer_storepress_slider_upsale").show();
        return false;   
        }
         if(jQuery('#exist_storepress_Slider').val()<3)
        {
         var new_service_add_val=parseInt(jQuery('#exist_storepress_Slider').val())+1;
         jQuery('#exist_storepress_Slider').val(new_service_add_val);  
        }
        }
		
		if(split_add_more_button_split=="Add New Spon")
        {
        if(jQuery('#exist_storepress_Sponsor').val()>=6)
        {
        jQuery(".customizer_storepress_sponsor_upsale").show();
        return false;   
        }
         if(jQuery('#exist_storepress_Sponsor').val()<6)
        {
         var new_service_add_val=parseInt(jQuery('#exist_storepress_Sponsor').val())+1;
         jQuery('#exist_storepress_Sponsor').val(new_service_add_val);  
        }
        }
		
		if(split_add_more_button_split=="Add New Data")
        {
        if(jQuery('#exist_storepress_Informations').val()>=2)
        {
        jQuery(".customizer_storepress_slider_info_upsale").show();
        return false;   
        }
         if(jQuery('#exist_storepress_Informations').val()<2)
        {
         var new_service_add_val=parseInt(jQuery('#exist_storepress_Informations').val())+1;
         jQuery('#exist_storepress_Informations').val(new_service_add_val);  
        }
        }
    ...
    storepress_theme_controls.on('click', '.social-repeater-general-control-remove-field', function () {
        ...
        if (typeof    jQuery(this).parent() !== 'undefined') {
            jQuery(this).parent().hide(500, function(){
                ...
                {
                jQuery('#exist_storepress_Information').val(parseInt(jQuery('#exist_storepress_Information').val())-1); 
				jQuery('#exist_storepress_Informations').val(parseInt(jQuery('#exist_storepress_Informations').val())-1);  				
                }
				if(split_delete_button_split=="Delete Slide")
                {
                jQuery('#exist_storepress_Slider').val(parseInt(jQuery('#exist_storepress_Slider').val())-1);  
                }
				if(split_delete_button_split=="Delete Spons")
                {
                jQuery('#exist_storepress_Sponsor').val(parseInt(jQuery('#exist_storepress_Sponsor').val())-1);  
                }
    ...
    storepress_theme_controls.on('keyup', '.customizer-repeater-title-control', function () {
        storepress_customizer_repeater_refresh_general_control_values();
    });
    ...
    storepress_theme_controls.on('keyup', '.customizer-repeater-subtitle-control', function () {
        storepress_customizer_repeater_refresh_general_control_values();
    });
	
	storepress_theme_controls.on('keyup', '.customizer-repeater-subtitle2-control', function () {
        storepress_customizer_repeater_refresh_general_control_values();
    });
	
	 storepress_theme_controls.on('keyup', '.customizer-repeater-btn3-control', function () {
        storepress_customizer_repeater_refresh_general_control_values();
    });
	

    storepress_theme_controls.on('keyup', '.customizer-repeater-shortcode-control', function () {
        storepress_customizer_repeater_refresh_general_control_values();
    });

    storepress_theme_controls.on('keyup', '.customizer-repeater-text-control', function () {
        storepress_customizer_repeater_refresh_general_control_values();
    });

    storepress_theme_controls.on('keyup', '.customizer-repeater-link-control', function () {
        storepress_customizer_repeater_refresh_general_control_values();
    });

    storepress_theme_controls.on('keyup', '.customizer-repeater-text2-control', function () {
        storepress_customizer_repeater_refresh_general_control_values();
    });
	
	storepress_theme_controls.on('keyup', '.customizer-repeater-button2-control', function () {
        storepress_customizer_repeater_refresh_general_control_values();
    });

    storepress_theme_controls.on('keyup', '.customizer-repeater-link2-control', function () {
        storepress_customizer_repeater_refresh_general_control_values();
    });
	
	 storepress_theme_controls.on('keyup', '.customizer-repeater-link3-control', function () {
        storepress_customizer_repeater_refresh_general_control_values();
    });
	
	storepress_theme_controls.on('change','.customizer-repeater-checkbox', function(){
    ...
    /*----------------- Socials Repeater ---------------------*/
    storepress_theme_controls.on('click', '.social-repeater-add-social-item', function (event) {
        event.preventDefault();
        var th = jQuery(this).parent();
        var id = 'customizer-repeater-social-repeater-' + storepress_customizer_repeater_uniqid();
        if (typeof th !== 'undefined') {
    ...
    storepress_theme_controls.on('click', '.social-repeater-remove-social-item', function (event) {
        event.preventDefault();
        var th = jQuery(this).parent();
        var repeater = jQuery(this).parent().parent();
        th.remove();
        storepress_customizer_repeater_refresh_social_icons(repeater);
        return false;
    });

    storepress_theme_controls.on('keyup', '.customizer-repeater-social-repeater-link', function (event) {
        event.preventDefault();
        var repeater = jQuery(this).parent().parent();
        storepress_customizer_repeater_refresh_social_icons(repeater);
        return false;
    ...
    storepress_theme_controls.on('change', '.customizer-repeater-social-repeater-container .icp', function (event) {
        event.preventDefault();
...
function storepressescapeHtml(string) {
    'use strict';
    //noinspection JSUnresolvedFunction
    string = String(string).replace(new RegExp('\r?\n', 'g'), '<br />');
    string = String(string).replace(/\\/g, '&#92;');
    return String(string).replace(/[&<>"'\/]/g, function (s) {
        return storepressentityMap[s];
    });
